...
============================================================
  ...
  Runs 24/7, scans odds every 30 min, logs to Supabase,
...
#   or paste values directly here for local testing
# ============================================================
...
SCAN_INTERVAL     = int(os.getenv("SCAN_INTERVAL",  "1800"))   # 30 minutes
...
def fetch_odds(league: str) -> list:
    ...
    try:
        ...
        r.raise_for_status()
        log.info(f"  {league}: {len(r.json())} matches")
        return r.json()
...
def parse_best_odds(match: dict, league: str) -> dict | None:
    ...
    for bk in match.get("bookmakers", []):
        ...
        for mkt in bk.get("markets", []):
            ...
            for o in mkt.get("outcomes", []):
                ...
                if n == home and p > best["home"]:
                    best["home"], best["home_bk"] = p, bk_name
                elif n == away and p > best["away"]:
                    best["away"], best["away_bk"] = p, bk_name
                elif n == "Draw" and p > best["draw"]:
                    best["draw"], best["draw_bk"] = p, bk_name
    ...
    return {
        ...
        "home_bookmaker": best["home_bk"],
        "away_bookmaker": best["away_bk"],
    ...
    html = f"""<html><body style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;padding:20px">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:14px">
        <thead><tr style="background:#f5f5f5">
          ...
          <th style="padding:10px 8px">Home</th><th style="padding:10px 8px">Draw</th>
          <th style="padding:10px 8px">Away</th>
          <th style="padding:10px 8px">If home wins</th><th style="padding:10px 8px">If away wins</th>
        </tr></thead>
        <tbody>{rows}</tbody>
      </table>
      <p style="margin-top:20px;font-size:12px;color:#aaa">Scanned {datetime.now().strftime('%Y-%m-%d %H:%M')} · Check dashboard for full history</p>
    </body></html>"""
    ...
    for league in LEAGUES:
        for raw in fetch_odds(league):
            ...
            if parsed["is_opportunity"]:
                ...
                log_opportunity(db, parsed)
        time.sleep(1)

    log_to_db(db, all_records)
